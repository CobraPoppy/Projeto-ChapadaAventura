...
db.serialize(() => {
    ...
    db.get("SELECT id FROM users WHERE email = ?", ["admin@chapada.com"], (err, row) => {
        ...
        if (!row) {
            ...
            db.run("INSERT INTO users (name, email, password_hash, role, created_at) VALUES (?, ?, ?, ?, ?)", ["Administrador", "admin@chapada.com", hash, "admin", now], (e) => {
                if (e)
...
app.post("/api/auth/register", (req, res) => {
    ...
    db.run("INSERT INTO users (name, email, password_hash, role, created_at) VALUES (?, ?, ?, ?, ?)", [name, email, hash, "user", now], function (err) {
        if (err) {
...
app.post("/api/auth/login", (req, res) => {
    ...
    db.get("SELECT id, name, email, password_hash, role, created_at FROM users WHERE email = ?", [email], (err, row) => {
        if (err) {
...
app.post("/api/contacts", (req, res) => {
    ...
    const now = new Date().toISOString();
    db.run("INSERT INTO contacts (name, email, phone, message, created_at) VALUES (?, ?, ?, ?, ?)", [name, email, phone || "", message, now], function (err) {
        if (err) {
